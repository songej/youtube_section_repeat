...
description: Privacy policy for Section Repeat for YouTube, reflecting version 1.9.29.
...
**Applies to:** Section Repeat for YouTube™ v1.9.29
...
Version 1.9.29 stores its random salt locally in `chrome.storage.local`. For compatibility with older installations, a profile that has no local salt may read a salt previously stored by an older version in `chrome.storage.sync` and copy it into local storage so existing saved sections remain reachable. Version 1.9.29 does not write new data to Chrome Sync.
...
- Clear all sections for the current video with `Q`.
